...
class CellProps(object):
    ...
    '''
    def __init__(self, centroid, sidelength, cellnum, row, column, model_top, geometry=None):
...
class CellPropsAll:
    def __init__(self):
        self.allcells = dict()
        self.centroids = dict()
        self.have_geometries = False

    def populate_cells(self, SFRdata):

        nod_file = 'D:/ATLData/Wbasin/output_usgdata/grid02qtg.nod'
        if SFRdata.gridtype == 'structured':
            ...
            print '\nreading in node attribute information from {}...'.format(nod_file)
            nodes = np.genfromtxt(nod_file, delimiter=' ', skiprows=1)
            knt = 0
            length = float(len(nodes))
            self.have_geometries = True
            for nod in nodes:
                knt +=1
                if nod[1] != 1:
                    print '\r{:.2f}%'.format(100*knt/length),
                    continue
                else:
                    ...
                    self.allcells[cellnum] = CellProps(centroid, minside, cellnum, row, column, model_top_elev, geometry=geometry)
                    print '\r{:.2f}%'.format(100*knt/length),

        else:
            # read grid info from grid shapefile
            print "\nreading in node attribute information from {}".format(SFRdata.MFgrid)
            print "only reading nodes in layer 1 and active nodes (node numbers != {})".format(SFRdata.inactive_node_number)
            shpobj = fiona.open(SFRdata.MFgrid)
            dict = {}
            knt = 0
            length = float(len(shpobj))
            self.have_geometries = True
            for line in shpobj:
                knt +=1
                # only read layer 1
...
class FragIDPropsAll:
    ...
    def populate(self, SFRdata, CELLdata=None):
        """
        read in the main COMID-related properties from the intersect file
        also read in model top elevation from Cell Properties Object
        ...
        if not CELLdata:
            raise NameError(
                "CellPropsAll.populate_cells not instantiated. Please instantiate in SFR_main file prior to "
                "running FragIDPropsAll.populate, and then pass its instance as an argument. CellProps provides a"
                "universal repository for storing information about the model grid that is independent of input "
                "grid type.")
        ...
        for reach in reaches:
            ...
            cellnum = reach.getValue(SFRdata.node_attribute)
            model_top = CELLdata.allcells[cellnum]
...
class SFRpreproc:
    ...
    def clip_and_join_Gridgen(self, SFRoperations):
        ...
        self.ofp.write(mlsdf.to_string(columns=['COMID'])) # right these dataframe column to log file
        df = df.drop(mlsdf.index, axis=0)
        ...
        print "\njoining PlusflowVAA and Elevslope tables to NHD Flowlines..."
        lsuffix = 'fl'
        ...
        print "\nAdding in start,end x,y and lengths..."
        ...
        print "\ncreating Fragment IDs (proto-reaches)"
    ...
    def intersect_DEM(self, SFRdata):
        ...
        with fiona.open(SFRdata.intersect) as input:
            schema = {'geometry': 'Point',
                      'properties': {'FragID': 'int'}}
            with fiona.open(SFRdata.intersect_points, "w", "ESRI Shapefile", schema) as output:

                for line in input:
                    vertices = line['geometry']['coordinates']
                    fragid = line['properties']['FragID']

                    for pnt in vertices:

                        output.write({'properties': {'FragID': fragid},
                                      'geometry': mapping(Point(np.array(pnt)))})

        shutil.copy(SFRdata.intersect[:-4]+'.prj', SFRdata.intersect_points[:-4]+'.prj')
...
class SFROperations:
    ...
    def intersect(self):
        ...
        # check to make sure that Flowlines has elevation fields
        # Gridgen preproc method doesn't do anything with Flowlines besides clip them
        fields = [f.name for f in arcpy.ListFields(self.SFRdata.Flowlines)]
        if 'MAXELEVSMO' not in fields:
            arcpy.MakeTableView_management(self.SFRdata.Elevslope, "Elevslope")
            arcpy.MakeFeatureLayer_management(self.SFRdata.Flowlines, "Flowlines")
            arcpy.AddJoin_management("Flowlines", "COMID", "Elevslope", "COMID", "KEEP_COMMON")
            arcpy.CopyFeatures_management("Flowlines", os.path.join(self.SFRdata.working_dir, 'temp.shp'))
            arcpy.CopyFeatures_management("Flowlines", self.SFRdata.NHD)
        else:
        ...
        if len(cutoff_comids) > 0:
            print 'See {} for information on COMIDs that have both ends cutoff'\
                .format(os.path.join(self.SFRdata.working_dir, 'boundary_manual_fix_issues.txt'))
        ...
        print "\nUpdating lengths and elevations for streams crossing the grid boundary to {0:s}".format(self.SFRdata.NHD)
        ...
        for comid in FragIDdata.allcomids:
            ...
            for i in range(1, len(end_has_start)):
                try:
                    ...
                    # ATL, 8/1/2014: I'm not sure what is going on in this loop,
                    # but it seems like this error shouldn't apply to the last fragment in the comid
                    # for Wbasin quadtree, this exception happened at FragID 280235, COMID 12568572
                    if orderedFragID[i-1] == endingFragID[0]:
                        continue
                    else:
...
class SFRoutput:
    ...
    def buildSFRshapefile2(self, CELLProps=None, FragIDdata=None):
        ...
        # Note: as of 7/29/14 node number and FragID are written to Mat 1,
        ...
        if CELLProps and FragIDdata:
            if CELLProps.have_geometries:
                print "making shapefile of SFR network from cell geometries..."
                input = Mat1[self.indat.node_attribute].values
                from_shp = False
        else:
            from_shp = True
        ...
        for node in input:
            ...
            for uniquereach in Mat1.ix[cellnum].index:
                ...
                output.write({'properties': {self.indat.node_attribute: Mat1.ix[(cellnum, uniquereach), self.indat.node_attribute],
